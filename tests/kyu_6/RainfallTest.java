...
public class RainfallTest {
    ...
    private static String[][] extractDataSol5612(String strng) {
    ...
    private static String findTownSol5612(String town, String strng) {
        String[][] arr = extractDataSol5612(strng);
        String[] fnd = null;
        for (String[] a : arr) {
            if (a[0].equals(town))
                fnd = a;
    ...
    private static double meanSol5612(String town, String strng) {
        String s = findTownSol5612(town, strng);
        if (s == null)
            return -1;
        double s1 = 0;
        int cnt = 0;
        ...
        for (String w : words) {
            String[] c = w.split(" ");
            double n = Double.parseDouble(c[1]);
            s1 += n;
            cnt++;
        }
        return s1 / cnt;
    ...
    @Test
    public void test3() {
        System.out.println("Random Tests: mean data");
        for (String s : shuffle(RainfallTest.towns)) {
            //System.out.println(s);
            assertFuzzyEquals(Rainfall.mean(s, RainfallTest.data), meanSol5612(s, RainfallTest.data));

        }
    }
    @Test
    public void test4() {
        System.out.println("Random Tests: variance data");
        for (String s : shuffle(RainfallTest.towns)) {
            //System.out.println(s);
            assertFuzzyEquals(Rainfall.variance(s, RainfallTest.data), varianceSol5612(s, RainfallTest.data));

        }
    }
    @Test
    public void test5() {
        System.out.println("Random Tests: mean data1");
        for (String s : shuffle(RainfallTest.towns)) {
            //System.out.println(s);
            assertFuzzyEquals(Rainfall.mean(s, RainfallTest.data1), meanSol5612(s, RainfallTest.data1));

        }
    }
    @Test
    public void test6() {
        System.out.println("Random Tests: variance data1");
        for (String s : shuffle(RainfallTest.towns)) {
            //System.out.println(s);
            assertFuzzyEquals(Rainfall.variance(s, RainfallTest.data1), varianceSol5612(s, RainfallTest.data1));
