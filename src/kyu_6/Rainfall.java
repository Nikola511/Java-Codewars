...
class Rainfall {
    static double[] rainfalls(String town, String str) {
        String[] strings = str.split("\n");
        String string = "";
        for(String s : strings)
            if(s.startsWith(town)) {
                string = s.replaceAll("[:a-zA-z\\s]", "");
                break;
            }
        return Arrays.stream(string.split(",")).mapToDouble(Double::parseDouble).toArray();
    }
    static double mean(String town, String str) {
        if(!str.contains(town + ":")) return -1;
        double[] doubleArray = rainfalls(town, str);
        return Arrays.stream(doubleArray).average().getAsDouble();
    }
    static double variance(String town, String str) {
        if(!str.contains(town + ":")) return -1;
        double[] doubleArray = rainfalls(town, str);
        double mean = Arrays.stream(doubleArray).average().getAsDouble();
        double total = 0;
